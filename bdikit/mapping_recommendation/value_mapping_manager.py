...
class ValueMappingManager:
    def __init__(
        ...
        column_mapping,
        target_domain,
        algorithm=MappingAlgorithm.EDIT.value,
    ):
    ...
    def calculate_coverage(self):
        ...
        sorted_results = sorted(
            self.mapping_results.items(), key=lambda x: x[1]["coverage"], reverse=True
        )
        total = 0
    ...
    def _match_values(self):
        ...
        for current_column, target_column in self.column_mapping.items():
            if self.target_domain[target_column] is None:
                continue
            target_values_dict = {
                x.lower(): x for x in self.target_domain[target_column]
            }
            ...
            self.mapping_results[current_column] = {
                "matches": None,
                "coverage": None,
                "unique_values": None,
                "unmatch_values": None,
            ...
            matches_lowercase = self.matcher_method.match(
                list(current_values_dict.keys()), list(target_values_dict.keys())
            )
            matches = []
            ...
            for current_value, target_value, similarity in matches_lowercase:
                matches.append(
                    (
                        current_values_dict[current_value],
                        target_values_dict[target_value],
                        similarity,
                    )
                )
            ...
            self.mapping_results[current_column]["unmatch_values"] = (
                current_values - match_values
            )
