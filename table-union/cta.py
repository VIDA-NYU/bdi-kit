...
class CTA:
    ...
    def get_column_type(self, context, model):
        ...
        messages=[
                ...
                {
                    "role": "user", 
                    "content": """ Please select the class from """ + self.labels + """ which best describes the context. Please respond only with the name of the class. Reply None if none of the classes are applicable.
                    \n CONTEXT: """ + context +  """ \n RESPONSE: \n"""},
            ],
        temperature=0.3)
        ...
        for filename in tqdm(os.listdir(self.input_dir), desc="Annotating columns"):
            if filename.endswith(".csv"):
                ...
                for col_index, col_name in enumerate(df.columns):
                    ...
                    if len(df[col_name].dropna()) > 15:
                        rows = df[col_name].dropna().sample(15).tolist()
                    else:
                        rows = df[col_name].dropna().tolist()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_dir", type=str, default="./data/temp_tables", help="The directory containing the tables.")
