...
PATH_TO_GDC_SCHEMA = join(dirname(__file__), "../gdc/gdc_schema.json")
...
class GDCSchema:
    ...
    def get_df_from_recommandations(self, recommandations):
        data_dict = {"parent": [], "column_name": [], "column_type": [], "column_description": [], "column_values": []}
        for parent, values in self.schema.items():
            for candidate in values["properties"].keys():
                if candidate in recommandations:
