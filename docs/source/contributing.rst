...
2. Define a class in the module to implements `BaseStandard`. This class should implement three methods:
   ...
   - `get_attribute_metadata()`: Returns a dictionary where the keys are attribute names and the values are dictionaries containing these mandatory fields for each attribute:
     `attribute_description`, `value_names`, and `value_descriptions`. Other fields can be included as well, but their values must be strings or lists of strings.
